  ...
  */
    init: function() {
        ...
        this.storage.initialize(function() {
            ...
            if (window.File && window.FileReader && window.FileList && window.Blob) {} else if (gentle.is_mobile) {
    ...
    loadLocalPlugins: function(callback) {
        ...
        me.storage.getItem('plugin_lists', function(plugin_list) {
            if (plugin_list && object_length(plugin_list.all) > 0) {
                ...
                $.each(plugin_list.all, function(k, v) {
                    plugins.load_on_start.push(k);
                });
  ...
  */
    ...
    do_annotate: function() {
        ...
        function submitTask() {
            ...
            sc.sequence.undo.addAction('addAnnotation', {
                label: 'add annotation',
                editing: false,
                action: 'addAnnotation',
                feature_id: sc.sequence.features.length,
                feature: clone(ann)
            });
        ...
        dialogContainer.load("public/templates/aad_annotation_dialog.html", function() {
            ...
            $("#aad_annotation_form input[type=submit]").click(function() {
                submitTask();
            });
            $("#aad_name").keypress(function(e) {
                if (e.keyCode === 13) submitTask();
            });
        });
    ...
    do_selection_info: function() {
        ...
        dialogContainer.load("public/templates/selection_info_dialog.html", function() {
            ...
            var mol_weight = (tallies['A'] * 313.2) + (tallies['C'] * 289.2) + (tallies['G'] * 329.2) + (tallies['T'] * 304.2);
    ...
    markDropArea: function(evt, mode) {
        ...
        $('#drop_zone').css({
            'background-color': (mode ? '#CCCCCC' : 'white')
        });
    },
